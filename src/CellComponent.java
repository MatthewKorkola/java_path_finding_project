...
public class CellComponent extends JComponent  {
	...
	private String imgInitial = "house.jpg";
	private String imgInitialPushed = "house_pushed.jpg";
	private String imgInitialPopped = "house_popped.jpg";
	private String imgExit = "middlesex.jpg";
	private String imgExitPushed = "middlesex_pushed.jpg";
	private String imgExitPopped = "middlesex_popped.jpg";
	private String imgDonut = "donut.jpg";
	private String imgDonutPushed = "donut_pushed.jpg";
	private String imgDonutPopped = "donut_popped.jpg";
	...
	private String imgCovid = "covid.jpg";
	...
	protected void paintComponent(Graphics g) {
		...
		if (c == CellColours.BLOCK) {
		...
		} else if (c == CellColours.INITIAL_PROCESSED || c == CellColours.INITIAL_POPPED) {
			// Initial and destination cells when they have been reached
			displayed = true;
			Image img;
			if (c == CellColours.INITIAL_PROCESSED)
			...
			g2d.setPaint(gp);
			g2d.fillPolygon(cellPoly);
			img = new ImageIcon(imgCovid).getImage();
			//g2d.drawImage(img, width / 5, height / 5, 3 * width / 5, 3 * height / 5, null);
			g2d.drawImage(img, 0, 0, width, width, null);

			displayed = true;
		
		} else if (c == CellColours.EXIT || c == CellColours.EXIT_PUSHED || c == CellColours.EXIT_POPPED) {
			// Destination cell.
			Image img;
			if (c == CellColours.EXIT) {
				// EXIT
				img = new ImageIcon(imgExit).getImage();
			} else if (c == CellColours.EXIT_PUSHED) {
				// EXIT_PUSHED
				img = new ImageIcon(imgExitPushed).getImage();
			} else {
				// EXIT_POPPED
				img = new ImageIcon(imgExitPopped).getImage();
			}
		...
		} else if (c == CellColours.DONUT || c == CellColours.DONUT_PUSHED || c == CellColours.DONUT_POPPED) {
			// Destination cell.
			Image img;
			if (c == CellColours.DONUT) {
				// EXIT
				img = new ImageIcon(imgDonut).getImage();
			} else if (c == CellColours.DONUT_PUSHED) {
				// EXIT_PUSHED
				img = new ImageIcon(imgDonutPushed).getImage();
			} else {
				// EXIT_POPPED
				img = new ImageIcon(imgDonutPopped).getImage();
			}
